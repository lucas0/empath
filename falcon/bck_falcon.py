...
df = pd.read_csv(cwd+"/verb_sentences.csv")
...
if num_entries != len(df):
    ...
    for i in tqdm(range(0, len(df), batch_size)):
        ...
        for idx, e in enumerate(meta):
            ...
            trunk_size = min(len(e_words), 5000)
            meta[idx]["sentences"] = ' '.join(e_words[:trunk_size])
...
model_name_or_path = "ichitaka/falcon-40b-instruct-8bit"
